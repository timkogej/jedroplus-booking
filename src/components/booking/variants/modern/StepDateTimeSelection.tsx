...
export default function StepDateTimeSelection() {
  ...
  // selectedDate is Date | null from the hook
  const selectedDateObj: Date | null = selectedDate instanceof Date ? selectedDate : null

  // Format selectedDate as string for display purposes
  const selectedDateStr: string | null = selectedDateObj
    ? format(selectedDateObj, 'yyyy-MM-dd')
    : null

  useEffect(() => {
    if (selectedDateObj) {
      if (!isSameMonth(selectedDateObj, currentMonth)) {
          ...
          >
            <div className="inline-flex items-center gap-3 px-6 py-3 rounded-2xl bg-white/10 backdrop-blur-xl border border-white/20">
              ...
              <span className="text-white font-display">
                {format(new Date(selectedDateStr), 'd. MMMM yyyy', { locale: sl })} ob {selectedTime}
              </span>
            </div>
          </motion.div>
        )}
